...
export const ImageUpload = () =>{
  ...
  const handleSubmit = e => {
    ...
    newFormData.append('image', postImage.image);
    newFormData.append('uploader', formData.uploader);
    newFormData.append('title', formData.title);
    
    const url = 'http://localhost:8000/appexample/upload/';  //work for all types of urls
    // let url = 'http://localhost:8000/appexample/calculate';
    axios
     
        .post(url, newFormData, { headers: {'Content-Type': 'multipart/form-data'}})
        .then((res) => {
